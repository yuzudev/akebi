...
import { config, handler } from './config';
...
export type RecGenerator<T = unknown> = Generator<T, void | RecGenerator<T>>;

export function* load<T extends Module>(dir: string): RecGenerator<T> {
    for (const file of readdirSync(dir)) {
        ...
        const mod = require(`${process.cwd()}/${dir}/${file}`);
